      ...
      <main className={`${styles.page} bg-white text-[#17364f]`}>
        <section className="px-5 pb-14 pt-12 text-center sm:px-6 sm:pb-20 sm:pt-16">
          <ScrollReveal>
            ...
            <p className="mx-auto mt-5 max-w-xl text-[11px] leading-relaxed text-[#718697] sm:text-lg">
          ...
          <div className="mx-auto max-w-5xl">
            <ScrollReveal>
              <div className="text-center">
                <h2 className="text-2xl font-bold tracking-[-0.02em] sm:text-3xl">Our SaaS Products</h2>
                <p className="mt-2 text-base text-[#718697]">Ready-to-deploy solutions for modern businesses</p>
              </div>
            </ScrollReveal>
                  ...
                  <ScrollReveal key={product.name} delay={index * 100}>
                    ...
                    <article className="relative h-full rounded-lg border border-[#cfe1e9] bg-white p-5 transition duration-300 hover:-translate-y-1 hover:border-[#08afe8] hover:shadow-[0_14px_35px_rgba(8,174,232,0.13)]">
                      ...
                      <div className="mt-5 grid gap-x-5 gap-y-2 sm:grid-cols-2">
                        {product.features.map((feature) => <p key={feature} className="flex items-start gap-1 text-[14px] text-[#5f7587]"><Check className="mt-px h-2.5 w-2.5 shrink-0 text-[#08afe8]" /> {feature}</p>)}
                      </div>
                      <div className="mt-3 flex flex-wrap gap-1.5">{product.badges.map((badge) => <span key={badge} className="rounded bg-[#eaf8fc] px-2 py-1 text-[12px] font-medium text-[#25768e]">{badge}</span>)}</div>
                      <p className="mt-3 text-[14px] font-semibold text-[#17364f]">{product.price}</p>
                      <div className="mt-5 flex flex-wrap gap-2">
                        <span className="rounded-md bg-[#08b7f2] px-4 py-2 text-[14px] font-bold text-white transition group-hover:bg-[#079fd5]">{product.action} <ArrowRight className="ml-1 inline h-3 w-3" /></span>
                        <span className="rounded-md border border-[#08789e] px-4 py-2 text-[14px] font-semibold text-[#075a7c]">View Pricing</span>
                      </div>
                    </article>
                    </Link>
                  </ScrollReveal>
        ...
        <section id="custom" className="bg-[#f7fafc] px-6 py-14 sm:py-20">
          <div className="mx-auto max-w-5xl">
            <ScrollReveal><div className="text-center"><h2 className="text-2xl font-bold tracking-[-0.02em] sm:text-3xl">Custom Development Services</h2><p className="mt-2 text-base text-[#718697]">Enterprise-grade solutions tailored to your needs</p></div></ScrollReveal>
              ...
              {customServices.map(([title, text, Icon, tags], index) => (
                <ScrollReveal key={title} delay={index * 70}>
                  ...
                  <article className="h-full rounded-lg border border-[#cfe1e9] bg-white p-5 transition duration-300 hover:-translate-y-1 hover:border-[#08afe8] hover:shadow-[0_14px_30px_rgba(8,174,232,0.1)]">
                    ...
                    <div className="mt-3 flex flex-wrap gap-1">{tags.map((tag) => <span key={tag} className="rounded border border-[#d9e8ee] px-1.5 py-1 text-[12px] text-[#718697]">{tag}</span>)}</div>
                  </article>
                  </Link>
                </ScrollReveal>
              ))}
        ...
        <section className="px-6 py-14 sm:py-20">
          <div className="mx-auto max-w-5xl">
            <ScrollReveal><div className="text-center"><h2 className="text-2xl font-bold tracking-[-0.02em] sm:text-3xl">How We Work</h2><p className="mt-2 text-base text-[#718697]">Flexible engagement models to suit your needs</p></div></ScrollReveal>
              ...
              {workModels.map(([title, text, timeline, investment, items], index) => (
                <ScrollReveal key={title} delay={index * 90}>
                  <article className={`h-full rounded-lg border p-5 ${index === 1 ? "border-[#056b86] bg-[#056b86] text-white" : "border-[#cfe1e9] bg-white"}`}>
                    <h3 className="text-base font-bold">{title}</h3><p className={`mt-2 text-[14px] leading-relaxed ${index === 1 ? "text-white/80" : "text-[#718697]"}`}>{text}</p>
                    <p className="mt-4 text-[13px] font-semibold uppercase text-[#08afe8]">Timeline</p><p className="text-sm font-bold">{timeline}</p>
                    <p className="mt-3 text-[13px] font-semibold uppercase text-[#08afe8]">Investment</p><p className="text-sm font-bold">{investment}</p>
                    <ul className="mt-4 space-y-2">{items.map((item) => <li key={item} className="text-[14px]">◉ {item}</li>)}</ul>
                    <Link href="/contact" className={`mt-5 block rounded-md px-3 py-2 text-center text-[14px] font-bold ${index === 1 ? "bg-[#08b7f2] text-white" : "border border-[#08789e] text-[#075a7c]"}`}>{index === 1 ? "Get Quote" : index === 2 ? "Subscribe" : "Discuss Your Project"} →</Link>
                  </article>
                </ScrollReveal>
              ))}
            </div>
        ...
        <section className="bg-[#f7fafc] px-6 py-14 sm:py-20">
          <div className="mx-auto max-w-5xl">
            <ScrollReveal><div className="text-center"><h2 className="text-2xl font-bold tracking-[-0.02em] sm:text-3xl">Our Development Process</h2><p className="mt-2 text-base text-[#718697]">A proven methodology for successful project delivery</p></div></ScrollReveal>
            <div className="mt-10 grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
              {process.map(([number, title, text], index) => (
                <ScrollReveal key={number} delay={index * 80}><article className="text-center"><div className="text-3xl font-bold text-[#b9eafa]">{number}</div><h3 className="mt-3 text-base font-bold">{title}</h3><p className="mt-2 text-[14px] leading-relaxed text-[#718697]">{text}</p></article></ScrollReveal>
              ))}
            </div>
          </div>
        </section>
        ...
        <section className="bg-[#056b86] px-6 py-14 text-center text-white sm:py-20">
          <ScrollReveal><h2 className="text-2xl font-bold">Ready to Start Your Project?</h2><p className="mt-3 text-[15px] text-white/80">Let&apos;s discuss your requirements and find the perfect solution for your business.</p><Link href="/contact" className="mt-6 inline-flex rounded-md bg-[#08b7f2] px-5 py-2 text-[15px] font-bold hover:bg-[#079fd5]">Get a Free Consultation →</Link></ScrollReveal>
        </section>
      </main>
      <Footer />
    </>
