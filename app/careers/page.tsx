      ...
      <main className={`${styles.page} bg-white text-[#17364f]`}>
        <section className="px-5 pb-12 pt-12 text-center sm:px-6 sm:pb-16 sm:pt-16">
          <ScrollReveal>
            ...
            <p className="mx-auto mt-3 max-w-xl text-[10px] leading-relaxed text-[#718697] sm:text-sm">Build the future of enterprise software with us. We’re looking for passionate individuals who want to make a real impact.</p>
          </ScrollReveal>
          <div className="mx-auto mt-14 max-w-5xl">
            <ScrollReveal><h2 className="text-xl font-bold sm:text-2xl">Why Work With Us?</h2><p className="mt-2 text-[9px] text-[#718697]">Benefits that matter for your career and life</p></ScrollReveal>
            <div className="mt-8 grid gap-7 sm:grid-cols-4">
              {benefits.map(([title, text, Icon], index) => <ScrollReveal key={title} delay={index * 80}><article className={styles.benefit}><div className={styles.icon}><Icon className="h-4 w-4" /></div><h3 className="mt-3 text-[9px] font-bold">{title}</h3><p className="mt-2 text-[7px] leading-relaxed text-[#718697]">{text}</p></article></ScrollReveal>)}
            </div>
          </div>
        </section>
        ...
        <section className="bg-[#f7fafc] px-5 py-12 sm:px-6 sm:py-16">
          <div className="mx-auto max-w-4xl">
            <ScrollReveal><div className="text-center"><h2 className="text-xl font-bold sm:text-2xl">Open Positions</h2><p className="mt-2 text-[9px] text-[#718697]">Find your perfect role and apply today</p></div></ScrollReveal>
            <div className="mt-8 space-y-3">
              {openings.map((job, index) => <ScrollReveal key={job.title} delay={index * 70}><article className={styles.jobCard}><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><h3 className="text-[10px] font-bold text-[#092c48] sm:text-sm">{job.title}</h3><p className="mt-1 text-[7px] text-[#718697]">{job.type} <span className="mx-1">•</span> {job.location}</p></div><button type="button" onClick={() => applyForRole(job.title)} className={styles.applyButton}>Apply Now <ArrowRight className="h-2.5 w-2.5" /></button></div><h4 className="mt-3 text-[7px] font-bold text-[#17364f]">Responsibilities:</h4><ul className="mt-1 space-y-0.5">{job.responsibilities.map((item) => <li key={item} className="flex gap-1 text-[7px] text-[#718697]"><CheckCircle2 className="mt-px h-2.5 w-2.5 shrink-0 text-[#08afe8]" />{item}</li>)}</ul><div className="mt-2 flex flex-wrap gap-1">{job.tags.map((tag) => <span key={tag} className={styles.tag}>{tag}</span>)}</div></article></ScrollReveal>)}
            </div>
          </div>
        </section>

        <section className="px-5 py-12 text-center sm:px-6 sm:py-16">
          <ScrollReveal><h2 className="text-xl font-bold sm:text-2xl">How to Apply</h2><p className="mt-2 text-[9px] text-[#718697]">Our simple 3-step hiring process</p></ScrollReveal>
          <div className="mx-auto mt-8 grid max-w-3xl gap-8 sm:grid-cols-3">{[["1", "Submit Application", "Fill out the form or email your CV to careers@abhivorn.com"], ["2", "Technical Interview", "Code review or project discussion based on your role"], ["3", "Final Round", "Meet the founders and discuss culture fit"]].map(([number, title, text], index) => <ScrollReveal key={number} delay={index * 90}><article><div className={styles.step}>{number}</div><h3 className="mt-2 text-[9px] font-bold">{title}</h3><p className="mx-auto mt-1 max-w-[160px] text-[7px] leading-relaxed text-[#718697]">{text}</p></article></ScrollReveal>)}</div>
          <p className="mt-6 text-[8px] text-[#718697]">Average timeline: <strong className="text-[#17364f]">1-2 weeks</strong></p>
        </section>

        <section id="apply" className="bg-[#f7fafc] px-5 py-12 sm:px-6 sm:py-16">
          <ScrollReveal><div className="text-center"><h2 className="text-xl font-bold sm:text-2xl">Apply Now</h2><p className="mt-2 text-[9px] text-[#718697]">Take the first step towards your new career</p></div></ScrollReveal>
          <ScrollReveal className="mx-auto mt-8 max-w-lg">
            <form onSubmit={(event) => { event.preventDefault(); setSubmitted(true); }} className={styles.form}>
              {submitted && <div className="mb-3 rounded border border-green-200 bg-green-50 p-2 text-[9px] text-green-700">Application received. We’ll be in touch soon.</div>}
